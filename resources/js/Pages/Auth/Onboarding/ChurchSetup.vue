...
const submit = () => {
    form.post('/admin/onboarding/church', {
        forceFormData: true,
    });
};
            ...
            <div class="setup-form-wrapper">
                ...
                <div class="setup-logo">
                    ...
                    <div>
                        ...
                        <p class="setup-logo-sub">{{ t('onboarding.churchSetupSubtitle') }}</p>
                    </div>
                </div>
                ...
                <div class="setup-heading">
                    <h2>{{ t('onboarding.churchTitle') }}</h2>
                    <p>{{ t('onboarding.churchDesc') }}</p>
                </div>
                ...
                <!-- Form -->
                <form @submit.prevent="submit" class="church-form">
                    <!-- Logo upload -->
                    <div class="form-group">
                        <label>{{ t('onboarding.logo') }}</label>
                        <div class="logo-upload-zone" @click="logoInput?.click()">
                            <input ref="logoInput" type="file" accept="image/jpeg,image/png,image/webp" class="hidden-input" @change="onLogoSelected" />
                            <div v-if="logoPreview" class="logo-preview">
                                <img :src="logoPreview" alt="Logo preview" />
                                <button type="button" class="logo-remove" @click.stop="removeLogo">
                            ...
                            <div v-else class="logo-placeholder">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="M21 15l-5-5L5 21"/></svg>
                                <span>{{ t('onboarding.logoUpload') }}</span>
                                <span class="logo-hint">{{ t('onboarding.logoHint') }}</span>
                            </div>
                        </div>
                    ...
                    <!-- Church name -->
                    <div class="form-group">
                        <label for="name">{{ t('onboarding.churchName') }} *</label>
                        <div class="input-wrapper" :class="{ error: form.errors.name }">
                            <div class="input-icon">
                                <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path d="M3 21h18M5 21V7l7-4 7 4v14M9 21v-6h6v6"/></svg>
                            </div>
                            <input id="name" v-model="form.name" type="text" required :placeholder="t('onboarding.churchNamePlaceholder')" />
                        </div>
                    ...
                    <div class="form-row">
                        <!-- Abbreviation -->
                        <div class="form-group">
                            <label for="abbreviation">{{ t('onboarding.abbreviation') }}</label>
                            <div class="input-wrapper" :class="{ error: form.errors.abbreviation }">
                                <div class="input-icon">
                                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path d="M4 7V4h16v3M9 20h6M12 4v16"/></svg>
                                </div>
                                <input id="abbreviation" v-model="form.abbreviation" type="text" :placeholder="t('onboarding.abbreviationPlaceholder')" />
                            </div>
                            <p v-if="form.errors.abbreviation" class="field-error">{{ form.errors.abbreviation }}</p>
                        </div>
                        <!-- Visionary -->
                        <div class="form-group">
                            <label for="visionary_name">{{ t('onboarding.visionary') }}</label>
                            <div class="input-wrapper" :class="{ error: form.errors.visionary_name }">
                                <div class="input-icon">
                                ...
                                <input id="visionary_name" v-model="form.visionary_name" type="text" :placeholder="t('onboarding.visionaryPlaceholder')" />
                            </div>
                            <p v-if="form.errors.visionary_name" class="field-error">{{ form.errors.visionary_name }}</p>
                        </div>
                    </div>

                    <!-- Description -->
                    <div class="form-group">
                        <label for="description">{{ t('onboarding.description') }}</label>
                        <textarea id="description" v-model="form.description" rows="2" class="textarea" :class="{ error: form.errors.description }" :placeholder="t('onboarding.descriptionPlaceholder')"></textarea>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group">
                            ...
                            <div class="input-wrapper" :class="{ error: form.errors.address }">
                                <div class="input-icon">
                                    <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                                </div>
                                <input id="address" v-model="form.address" type="text" :placeholder="t('onboarding.addressPlaceholder')" />
                            </div>
        ...
        <div class="setup-right">
            ...
            <div class="right-content">
                ...
                <h2 class="right-title">
                    {{ t('onboarding.heroTitle1') }}<br>
                    <span>{{ t('onboarding.heroTitle2') }}</span>
                </h2>
                <p class="right-desc">{{ t('onboarding.heroDesc') }}</p>
                ...
                <div class="right-steps">
                    ...
                    <div class="right-step active">
                        ...
                        <div class="rs-text">
                            <strong>{{ t('onboarding.heroStep3') }}</strong>
                            <span>{{ t('onboarding.heroStep3Desc') }}</span>
                        </div>
                    </div>
                </div>
            </div>
...
.logo-upload-zone:hover { border-color: #6B4EAF; background: rgba(107,78,175,0.02); }
...
.logo-remove:hover { background: #dc2626; transform: scale(1.1); }
.logo-remove svg { width: 12px; height: 12px; }
.logo-placeholder {
    display: flex; flex-direction: column; align-items: center; gap: 6px;
    color: #9ca3af;
}
.logo-placeholder svg { width: 32px; height: 32px; }
.logo-placeholder span { font-size: 13px; font-weight: 600; color: #6b7280; }
.logo-hint { font-size: 11px; font-weight: 400; color: #9ca3af !important; }

/* Form */
.church-form { display: flex; flex-direction: column; gap: 12px; }
...
.textarea {
    width: 100%; padding: 10px 14px; font-size: 13px; color: #111827;
    background: #f9fafb; border: 2px solid #e5e7eb; border-radius: 12px;
    outline: none; font-family: inherit; resize: vertical; transition: all .25s;
}
.textarea:focus { border-color: #6B4EAF; background: #fff; box-shadow: 0 0 0 4px rgba(107,78,175,0.08); }
...
:root.dark .textarea { background: #1f2937; border-color: #374151; color: #f3f4f6; }
:root.dark .textarea:focus { border-color: #8b6fcf; background: #1a1a2e; }
...
:root.dark .logo-upload-zone { border-color: #374151; background: #1f2937; }
:root.dark .logo-upload-zone:hover { border-color: #8b6fcf; background: rgba(139,111,207,0.05); }
:root.dark .logo-placeholder span { color: #94a3b8; }
:root.dark .logo-preview img { border-color: #374151; }
