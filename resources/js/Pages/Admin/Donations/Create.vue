...
<template>
    ...
    <AdminLayout :title="t('donationCreate.title')">
        <div class="max-w-lg space-y-4">
            ...
            <!-- Sandbox banner -->
            <div v-if="isSandbox" class="flex items-center gap-2.5 px-3.5 py-2.5 bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-xl text-xs font-medium text-orange-700 dark:text-orange-400">
            ...
            >
                ...
                <div v-if="trackingState === 'pending'" class="bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-2xl p-5 text-center space-y-3">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-amber-800 dark:text-amber-300">{{ t('donationCreate.paymentInitiated') }}</h3>
                ...
                <div v-else-if="trackingState === 'completed'" class="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-200 dark:border-emerald-800 rounded-2xl p-5 text-center space-y-3">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-emerald-800 dark:text-emerald-300">{{ t('donationCreate.successTitle') }}</h3>
                ...
                <div v-else-if="trackingState === 'failed'" class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-2xl p-5 text-center space-y-3">
                    ...
                    <div>
                        <h3 class="text-sm font-bold text-red-800 dark:text-red-300">{{ t('donationCreate.paymentFailed') }}</h3>
            ...
            <form v-if="trackingState === 'idle'" @submit.prevent="submit" class="bg-white dark:bg-gray-800 rounded-2xl border border-gray-200/60 dark:border-gray-700 shadow-sm">
                <!-- Recipient badge -->
                <div class="px-5 pt-5 pb-3">
                    <div class="inline-flex items-center gap-2 px-3 py-1.5 bg-primary/5 rounded-lg border border-primary/10">
                        ...
                        <span class="text-xs font-semibold text-gray-700 dark:text-gray-300">{{ t('donationCreate.recipientPlatform') }}</span>
                    </div>
                </div>
                ...
                <div class="px-5 pb-5 space-y-4">
                    ...
                    <div class="grid grid-cols-5 gap-3">
                        <div class="col-span-2 space-y-1.5">
                            <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400">{{ t('donationCreate.country') }}</label>
                            <select
                                v-model="form.country_code"
                                class="w-full px-3 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-sm dark:text-gray-200 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition"
                            >
                        ...
                        <div class="col-span-3 space-y-1.5">
                            <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400">{{ t('donationCreate.amount') }}</label>
                            <div class="relative">
                                <input
                                    v-model="form.amount"
                                    type="number"
                                    ...
                                    class="w-full px-3 py-2.5 pr-14 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-sm dark:text-gray-200 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition"
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400">{{ t('donationCreate.phoneNumber') }}</label>
                        <input
                        ...
                        <p v-else class="text-[11px] text-gray-400 dark:text-gray-500">{{ t('donationCreate.phoneHint') }}</p>
                    </div>

                    <!-- Message -->
                    <div class="space-y-1.5">
                        <label class="block text-xs font-semibold text-gray-600 dark:text-gray-400">{{ t('donationCreate.message') }} <span class="text-gray-400 dark:text-gray-500 font-normal">({{ t('common.optional') }})</span></label>
                        <textarea
                            v-model="form.message"
                            rows="2"
                            :placeholder="t('donationCreate.messagePlaceholder')"
                            maxlength="500"
                            class="w-full px-3 py-2.5 bg-gray-50 dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-xl text-sm dark:text-gray-200 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition resize-none"
                        ></textarea>
                    </div>

                    <!-- Payment error -->
                    <div v-if="form.errors.payment" class="flex items-center gap-2.5 p-3 bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-xl">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-1">
                        <Link
                            ...
                            class="px-4 py-2.5 text-sm font-semibold text-gray-600 dark:text-gray-300 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-xl transition-colors"
                        >
                            {{ t('common.cancel') }}
                        </Link>
                        <button
